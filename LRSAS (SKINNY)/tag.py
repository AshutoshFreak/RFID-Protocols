...
fid = 0xaef923103db0b86407ebbab0
protocol_key = 0xdd0f5c30fe0e744c46de527d1e58d4ec
ID = 0x6c105a3b14cc093297b73491
...
def authenticate(m1m2):
    m1 = int(m1m2[:len(m1m2)//2])
    m2 = int(m1m2[len(m1m2)//2:])
    r_dash = m1 ^ fid
    p = SkinnyCipher(protocol_key)
    m2_dash = p.encrypt(fid^ID^r_dash)
    if m2_dash == m2:
        m3_dash = p.encrypt(m2_dash^r_dash)
        return m3_dash
    else:
        print(m2, m2_dash)
        print('Authentication Failed of reader')
        exit(1)
...
if __name__ == '__main__':
    ...
    if m1m2 == '0':
        print('Identification failed of tag')
        exit(1)
    ...
    if adr == 0:
        print('Authentication failed of tag')
        exit(1)

    # Update Phase
    fid = int(m1m2[:len(m1m2)//2])
    # Generate new key
    protocol_key = key_schedule(protocol_key)
